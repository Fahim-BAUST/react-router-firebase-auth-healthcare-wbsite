...
const Login = () => {
    const { error, signInUsingGoogle, user, signInUsingEamilAndPass } = useAuth();
    ...
    return (
        <div className="login container text-center  mx-auto border border-1 mt-5 rounded-3 ">
            ...
            <form className="text-start " onSubmit={handleSignIn}>
                ...
                <input type="email" onBlur={handleEmailChange} className="form-control border border-secondary" id="inputEmail4" />

                <label htmlFor="inputPassword4" style={{ fontSize: "13px" }} className="form-label fw-bold ">Password</label>
                <input type="password" onBlur={handlePasswordChange} className="form-control border border-secondary" id="inputPassword4" />
